...
| 2 | Trainer에서 학습 요청 (사전 테스트 및 자원 할당 포함) | 사용자가 Trainer 화면에서 개발 완료된 App을 선택하고, 리소스·학습 파라미터를 설정한다. 제출 전 사전 테스트를 실행하여 결과를 확인하고 요청에 첨부할 수 있다. 테스트 결과 첨부 시 관리자 승인이 간소화된다. App 선택, 자원 할당, 파라미터 설정, 사전 테스트, 제출이 Trainer에서 일괄 처리된다. |
| 3 | 테스트 확인 및 승인 | 개발자가 사전 테스트 결과를 첨부한 경우, 관리자는 첨부된 결과를 확인하고 승인한다. 미첨부 시 관리자가 직접 테스트를 실행한 후 승인한다. 개별 컴포넌트의 자원 요청량이 임계치 이상이면 관리자 승인 후 실행한다. |
...
Builder는 **2-level 네비게이션**과 **GUI 캔버스 기반** 편집을 제공한다:
- **App 목록 화면** (1단계): Builder 진입 시 기존 App 목록이 카드 형태로 표시되며, 기존 App 선택 또는 "새 App 만들기"를 통해 개발 화면에 진입한다.
- **App 개발 화면** (2단계): "← App 목록" 버튼으로 목록 복귀가 가능하며, 세 개의 탭으로 구성된다:
  - **App 관리**: App 생성, Task 추가/관리, App Import (다른 App의 Task 불러오기)
  - **Task 편집**: Task 선택 → 시각적 노드 카드로 Component 표시, 각 노드의 → 화살표 버튼으로 워크플로우 연결, 실시간 다이어그램 미리보기
  - **워크플로우 시각화**: Task 단위 SVG 다이어그램 전체 보기
...
| BL-07 | 워크플로우 구성 | Task 편집 탭에서 각 컴포넌트 노드의 → 화살표 버튼을 클릭하여 소스를 지정하고, 타겟 노드를 클릭하여 연결을 생성한다. 시각적 하이라이트로 소스/타겟을 구분한다. Task 단위로 분리됨 | P0 | Phase 1 |
| BL-08 | 워크플로우 시각화 | 정의된 워크플로우를 Task 단위 SVG 다이어그램으로 표시한다. Task 편집 탭에서 실시간 미리보기 제공, 워크플로우 시각화 탭에서 전체 보기 제공 | P0 | Phase 1 |
...
| BL-13 | App 목록 화면 | Builder 진입 시 기존 App을 카드 형태로 표시한다. App 이름, 버전, 상태, Task 수, Component 수, 생성일을 표시하며, 카드 클릭 시 해당 App의 편집 화면으로 진입한다 | P0 | Phase 1 |
| BL-14 | 기존 App 편집 | 기존 App의 이름, 버전, 환경 변수, Task 구성을 수정할 수 있다. specs 상태 배열의 해당 App을 직접 업데이트한다 | P0 | Phase 1 |
| BL-15 | 시각적 노드 카드 | Task 편집 시 컴포넌트를 컴팩트 노드 카드로 그리드 표시한다. 이름, 순서, GPU 정보, 상태 배지(정상/초과), IN/OUT 연결 배지를 표시하며, ✎ 버튼으로 상세 편집 패널을 펼칠 수 있다 | P0 | Phase 1 |
| BL-16 | 화살표 버튼 연결 | 각 컴포넌트 노드에 → 화살표 버튼을 제공한다. 클릭하면 소스로 지정(하이라이트)되고, 다른 노드 클릭 시 연결이 즉시 생성된다. 취소 버튼으로 연결 모드 해제 가능 | P0 | Phase 1 |
| BL-17 | 실시간 워크플로우 미리보기 | Task 편집 탭 하단에 워크플로우 다이어그램을 표시한다. 연결 추가/삭제 시 다이어그램이 자동으로 업데이트되어 탭 전환 없이 편집 결과를 즉시 확인할 수 있다 | P0 | Phase 1 |
...
개발 완료된 App에 대해 학습 파라미터와 리소스를 설정하여 학습 요청을 제출하는 전용 화면이다. Builder와 완전 분리된 별도 화면으로 운영된다. App 선택, 자원 할당, 학습 파라미터 설정, **사전 테스트 실행**, 요청 제출을 Trainer에서 일괄 처리한다.
...
| TR-06 | 학습 요청 제출 | App + 리소스 + 학습 파라미터를 묶어 학습 요청을 제출한다. 사전 테스트 결과가 있으면 함께 첨부된다. 기존 승인 프로세스에 진입 | P0 | Phase 1 |
| TR-07 | 학습 이력 조회 | 제출된 학습 요청의 이력과 상태를 조회한다 (대기, 승인대기, 실행 중, 완료) | P0 | Phase 1 |
| TR-08 | 학습 결과 비교 | 여러 학습 결과의 성능 메트릭을 비교한다 | P1 | Phase 2 |
| TR-09 | 사전 테스트 실행 | Trainer 화면에서 학습 요청 제출 전 최소 실행 테스트를 직접 실행한다. RL 워크로드는 최소 에피소드 실행 + 수렴 확인, 일반 워크로드는 최소 반복 실행 + 성공/실패 검증. App·리소스·파라미터 설정 완료 후 "테스트 실행" 버튼으로 수행 | P0 | Phase 1 |
| TR-10 | 테스트 결과 첨부 | 사전 테스트 통과(passed) 시 결과를 학습 요청에 첨부하여 제출한다. 첨부된 결과가 있으면 관리자가 별도 테스트 없이 확인 후 승인 가능. 테스트 실패(failed) 시 첨부 불가, 재실행 또는 미첨부 제출 선택 | P0 | Phase 1 |

### 4.2 사전 테스트 흐름

Trainer 화면에서 학습 요청을 제출하기 전, 개발자가 직접 최소 실행 테스트를 수행할 수 있다.

#### 사전 테스트 단계별 흐름

```
1. App 선택 + 리소스/파라미터 설정 완료
2. "테스트 실행" 버튼 클릭
3. 최소 실행 테스트 수행 (RL: 최소 에피소드, 일반: 최소 반복)
4. 테스트 결과 확인 (로그, 성능 지표, 수렴 여부)
5-A. 통과(passed) → "테스트 결과 첨부하여 제출" 또는 "미첨부 제출"
5-B. 실패(failed) → 설정 수정 후 재실행 또는 "미첨부 제출"
```
...
> **테스트 및 승인 프로세스 개요**
>
> Trainer에서 학습 요청을 제출하기 전, 개발자가 직접 사전 테스트를 실행하고 결과를 첨부할 수 있다. 개발자가 테스트 결과를 첨부한 경우, 관리자는 첨부된 결과를 확인(리뷰)하고 별도 테스트 없이 승인할 수 있다. 테스트 결과가 첨부되지 않은 경우에만 관리자가 직접 테스트를 실행한다. 개별 컴포넌트의 자원 요청량이 설정된 임계치 이상인 경우 관리자 승인 후 실행되며, 임계치 미만이면 즉시 실행된다. 자동 승인(임계치 미만) 워크로드에는 테스트가 적용되지 않는다.

**자원 임계치 승인 기준:** App을 구성하는 개별 컴포넌트 중, 어느 하나라도 GPU >= 4기 또는 메모리 >= 128GB이면 관리자 승인이 필요하다.

### 5.1 승인 프로세스 흐름

개발자 사전 테스트 첨부 여부에 따라 두 가지 흐름으로 분기한다:

#### 흐름 A: 개발자가 테스트 결과를 첨부한 경우

```
[Trainer] 사전 테스트 실행 → 통과 → 결과 첨부하여 요청 제출
→ [관리자] 첨부된 테스트 결과 확인(리뷰) → 승인/반려
```

- 관리자는 첨부된 결과(로그, 성능 지표, 수렴 여부)를 검토한다
- 결과가 충분하다고 판단되면 "확인 완료" 후 바로 승인한다
- 별도 테스트 실행 없이 승인 가능하다
- 필요 시 관리자가 추가 테스트를 실행할 수도 있다 (선택)

#### 흐름 B: 테스트 결과 미첨부인 경우

```
[Trainer] 테스트 없이 요청 제출
→ [관리자] 직접 테스트 실행 → 결과 확인 → 승인/반려
```

- 기존 v1.5와 동일한 흐름이다
- 관리자가 직접 테스트를 실행하고 결과를 확인한 후 승인한다

### 5.2 기능 명세

| ID | 기능명 | 설명 | 우선순위 | 구현 단계 |
|----|--------|------|----------|-----------|
| AP-01 | 학습 요청 목록 조회 | Trainer에서 제출된 App 학습 요청 목록을 조회한다 (상태: 대기/승인대기/승인/반려). 테스트 결과 첨부 여부를 배지로 표시한다 | P0 | Phase 1 |
| AP-02 | 요청 상세 검토 | App 스펙 파일 내용, 요청 자원량, 신청자 정보를 상세히 확인한다 | P0 | Phase 1 |
| AP-03 | 테스트 실행 (관리자) | 개발자가 테스트 결과를 첨부하지 않은 needsApproval 워크로드에 대해 관리자가 최소 실행 테스트를 수행한다. 개발자 첨부 결과가 있는 경우 관리자 테스트 실행은 선택사항이다 | P0 | Phase 1 |
| AP-04 | 테스트 결과 확인 | 테스트 결과 확인: RL은 에피소드별 보상 추이·수렴 그래프·최종 성능 지표, 일반은 반복 실행 로그·성공률·평균 실행 시간 표시. 개발자 첨부 결과와 관리자 실행 결과 모두 동일한 형식으로 표시한다 | P0 | Phase 1 |
| AP-05 | 승인/반려 처리 | 개별 컴포넌트의 자원 요청량이 임계치 이상이면 관리자 승인 후 실행 대기열에 진입. 개발자 첨부 테스트 결과를 확인(리뷰)한 경우 또는 관리자가 직접 테스트를 통과시킨 경우 승인 가능. 테스트 결과 없음(미첨부 + 관리자 미실행) 시 승인 불가 (경고 표시). 임계치 미만이면 즉시 실행. 반려 시 사유를 코멘트로 기록한다 | P0 | Phase 1 |
| AP-06 | 첨부 테스트 결과 리뷰 | 개발자가 첨부한 사전 테스트 결과를 관리자가 확인한다. 결과 로그, 성능 지표, 수렴 여부를 검토하고 "확인 완료" 처리한다. 확인 완료 시 reviewedBy, reviewedAt이 기록된다 | P0 | Phase 1 |
...
| 시나리오 | 테스트 실행 주체 | 관리자 행동 | 승인 가능 여부 |
|----------|------------------|-------------|----------------|
| 개발자 사전 테스트 통과 + 결과 첨부 | developer | 첨부 결과 확인(리뷰) 후 승인 | 리뷰 완료 시 승인 가능 |
| 개발자 사전 테스트 통과 + 미첨부 | developer (미첨부) | 관리자가 직접 테스트 실행 필요 | 관리자 테스트 통과 후 승인 가능 |
| 사전 테스트 미실행 | — | 관리자가 직접 테스트 실행 필요 | 관리자 테스트 통과 후 승인 가능 |
| 개발자 사전 테스트 실패 | developer (failed) | 관리자가 직접 테스트 실행 필요 | 관리자 테스트 통과 후 승인 가능 |
...
```json
{
  "loopTest": {
    ...
    "executedBy": "developer",
    "executedAt": "2026-02-25 09:45",
    "attachedToRequest": true,
    "reviewedBy": "admin",
    "reviewedAt": "2026-02-25 10:15"
  }
}
```
...
전체 워크플로우의 기본 형상을 완성한다. Builder로 App을 개발하고, Trainer에서 사전 테스트 및 학습 요청, 관리자 확인/승인, 대기열 관리, 실행, 결과 모델 조회까지 전체 흐름이 동작한다.
...
- Trainer 화면: App 선택, 리소스 요청, 학습 파라미터 설정, **사전 테스트 실행, 테스트 결과 첨부**, 학습 요청 제출, 이력 조회 (TR-01~TR-10)
- 테스트 확인 및 승인: 첨부 결과 리뷰 → 확인 후 승인 / 미첨부 시 관리자 직접 테스트 → 승인. 컴포넌트별 자원 임계치 확인
